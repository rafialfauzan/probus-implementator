@php
    use Carbon\Carbon;
    $req = $data->request;
    $rentang = ($req && $req->start_date && $req->end_date) ? Carbon::parse($req->start_date)->diffInDays(Carbon::parse($req->end_date)) : 0;
@endphp
<x-app-layout>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-[28px] shadow-md card w-full">
                <div class="card-body">
                    <h1 class="card-title text-3xl font-extrabold mb-4">Edit Update System</h1>
                    <form action="/editus/{{ $data->id }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="req" value="{{ $data->request_id }}" required>
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div class="flex flex-col space-y-4">
                                    ...
                                    <div class="card bg-white w-full shadow-md text-black border rounded-2xl">
                                        <div class="card-body p-4 sm:p-5">
                                            ...
                                            <div class="card-actions justify-start my-1">
                                                <div class="badge badge-secondary badge-lg text-white">{{ $req->outlet ? $req->outlet->nm_out : '-' }}</div>
                                            </div>
                                            <div class="card-actions justify-start my-1">
                                                <div class="badge badge-primary badge-lg text-white">{{ $req->tag ? $req->tag->name : '-' }}</div>
                                                <div class="badge badge-neutral badge-lg text-white">{{ $req->kategori ? $req->kategori->name : '-' }}</div>
                                            </div>
                                            <div class="space-y-1 text-sm mt-2">
                                                ...
                                                <div>
                                                    <p class="font-semibold text-gray-700"><i class="fa-solid fa-user-pen text-primary"></i> {{ $req->user ? $req->user->name : '-' }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" class="input rounded-xl input-bordered w-full" value="{{ old('title', $data->judul) }}" name="title" placeholder="Enter update system title..." required>
                                </div>

                                <!-- Link .exe -->
                                <div>
                                    <p class="font-bold">Link .exe</p>
                                    <input type="text" class="input rounded-xl input-bordered w-full" value="{{ old('linkexe', $data->link) }}" name="linkexe" placeholder="Enter .exe download link...">
                                </div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <!-- Outlet -->
                                    <div x-data="{
                                        selectedId: {{ json_encode(old('outlet', $data->outlet_id)) }},
                                        searchQuery: {{ json_encode(old('outl', $data->outlet ? $data->outlet->nm_out : '')) }},
                                        open: false,
                                        loading: false,
                                        results: [],
                                        searchOutlets() {
                                    ...
                                    <div>
                                        ...
                                        <select name="category" id="category" class="select select-bordered w-full rounded-xl">
                                            ...
                                            <option value="{{ $kategoriitem->id }}" {{ old('category', $data->kategori_id) == $kategoriitem->id ? 'selected' : '' }}>{{ $kategoriitem->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-3">
                                        ...
                                        <div class="h-[120px] relative border shadow-sm overflow-hidden rounded-xl group">
                                            <a onclick="return confirm('Are you sure you want to delete this data?')" href="/deleteimg/{{ $img->id }}" class="btn btn-error btn-xs text-white rounded-lg absolute top-2 right-2 z-10">✕</a>
                            ...
                            <div class="flex flex-col">
                                <div class="sticky top-0 z-20 bg-white pt-2 pb-2 mb-2 border-b border-gray-100 shadow-sm rounded-t-xl">
                                    ...
                                    <trix-toolbar id="trix-toolbar-editus"></trix-toolbar>
                                </div>
                                <input type="hidden" id="body" name="body" value="{{ old('body', $data->deskripsi) }}" required>
                                <div class="flex-1 flex flex-col">
                                    <trix-editor toolbar="trix-toolbar-editus" trix-attachment-remove input="body" class="bg-white rounded-xl border border-gray-300 p-3 min-h-[380px] flex-1 focus:outline-none"></trix-editor>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Submit Button -->
                        <div class="mt-6">
                            <button type="submit" class="btn btn-neutral bg-black w-full rounded-xl text-white hover:bg-gray-800">EDIT</button>
                        </div>
                    </form>
                </div>
            </div>
